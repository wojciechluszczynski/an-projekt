...
const allProjects = [
  { title: "Złota Harmonia", type: "Mieszkanie", area: 85, location: "Rzeszów", image: vizLivingBeige, href: "/zlota-harmonia" },
  { title: "Czarna Perła", type: "Dom", area: 180, location: "Podkarpacie", image: vizKitchenRattan, href: "/czarna-perla" },
  { title: "Bambusowa Oaza", type: "Salon", area: 65, location: "Małopolska", image: vizDiningFireplace, href: "/bambusowa-oaza" },
  { title: "Marmurowa Łazienka", type: "Łazienka", area: 12, location: "Krosno", image: vizBathroomMarble, href: "/marmurowa-lazienka" },
  { title: "Ciemna Sypialnia", type: "Sypialnia", area: 20, location: "Rzeszów", image: vizBedroomDark, href: "/ciemna-sypialnia" },
  { title: "Mural Sypialnia", type: "Sypialnia", area: 18, location: "Nowy Sącz", image: vizBedroomMural, href: "/mural-sypialnia" },
...
const Realizacje = () => {
  ...
  useEffect(() => {
    const timer = setInterval(() => setFeaturedIdx((i) => (i + 1) % featured.length), 5000);
  ...
  return (
    <main>
      ...
      <section className="relative h-screen w-full overflow-hidden">
        {featured.map((p, i) => (
          <div key={p.href} className={`absolute inset-0 transition-all duration-1000 ${i === featuredIdx ? "opacity-100 scale-100" : "opacity-0 scale-105 pointer-events-none"}`}>
            <img src={p.image} alt={p.title} className="w-full h-full object-cover" />
            <div className="absolute inset-0 bg-gradient-to-t from-foreground/70 via-foreground/20 to-transparent" />
          </div>
        ))}

        <div className="absolute bottom-16 md:bottom-20 left-6 md:left-16 lg:left-24 z-10 max-w-lg">
          ...
          <Link to={featured[featuredIdx].href} className="px-6 py-2.5 rounded-full bg-accent text-accent-foreground text-sm font-body hover:bg-accent/90 transition-colors">
            Zobacz projekt
          </Link>
        </div>

        {/* Arrows */}
        <div className="absolute bottom-16 md:bottom-20 right-6 md:right-16 lg:right-24 flex gap-3">
          <button onClick={() => setFeaturedIdx((i) => (i - 1 + featured.length) % featured.length)} className="w-11 h-11 rounded-full bg-dark-foreground/20 backdrop-blur-sm flex items-center justify-center text-dark-foreground hover:bg-dark-foreground/30 transition-colors">
            <ChevronLeft size={22} />
          </button>
          <button onClick={() => setFeaturedIdx((i) => (i + 1) % featured.length)} className="w-11 h-11 rounded-full bg-dark-foreground/20 backdrop-blur-sm flex items-center justify-center text-dark-foreground hover:bg-dark-foreground/30 transition-colors">
